...
import maid.model.api.caps.ModelCapsHelper;
import maid.model.api.model.motion.IMotion;
...
public abstract class ModelLittleMaidBase extends ModelMultiMMMBase {
     ...
     * メイドさんのカスタムモーション
     */
    public static List<IMotion> littleMaidMotions = new ArrayList<>();
...
//			Skirt.setRotationPoint(0.0F, 5.8F, 2.7F);
    ...
    protected void postMotionRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, IModelCaps entityCaps) {
        ...
        for (IMotion motion : littleMaidMotions) {
            //trueで以降のモーション制御を行わない
            if (motion.postRotationAngles(this, multiModelMotion, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch, scaleFactor, entityCaps)) {
                break;
            }
        }
    }
